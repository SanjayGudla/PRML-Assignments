...
x_train = list(X_train_ai)+list(X_train_bA)+list(X_train_chA)+list(X_train_dA)+list(X_train_tA)
c_train = [1]*len(X_train_ai)+[2]*len(X_train_bA)+[3]*len(X_train_chA)+[4]*len(X_train_dA)+[5]*len(X_train_tA)
x_test = list(X_dev_ai)+list(X_dev_bA)+list(X_dev_chA)+list(X_dev_dA)+list(X_dev_tA)
...
X_train_coast = scl.transform(X_train_ai)
X_train_forest = scl.transform(X_train_bA)
X_train_highway = scl.transform(X_train_chA)
X_train_mountain = scl.transform(X_train_dA)
X_train_opencountry =scl.transform(X_train_tA)
...
################################################################################################
### With PCA
pca_obj = PCA(x_train,39,10)
reduced_train = pca_obj.pca_func()
reduced_dev = pca_obj.project_test_array(x_test)
X_trains.append(np.array(reduced_train))
X_tests.append(np.array(reduced_dev))
#################################################################################################
...
tpr_arr = []
C_s = [39,10,4]
for (x_train,x_test,C) in zip(X_trains,X_tests,C_s):
    prediction,scores= Logistic(x_train,C,5,x_test,c_train).classify()
    p = list(prediction)
    count = 0
