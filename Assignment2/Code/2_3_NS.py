...
def calculate_total_sigma2I(X_train):
    res1 = calculate_individ_sigma(X1_train)
    res2 = calculate_individ_sigma(X2_train)
    res3 = calculate_individ_sigma(X3_train)
    res = res1 + res2 + res3
    n = np.size(X_train)
    res = res / n
    ans = (np.identity(2, dtype=float)) * res
    return ans
...
(sigma_total2I) = calculate_total_sigma2I(X_train)
########################################################################################################################################
##case 3:- Naive Bayes with C1 = C2 = C3 = (Sigma^2)I.
#
#
...
def calculate_prob(c, x, y):
    ...
    g -= np.log(math.sqrt(LA.det(sigma_total2I)))
    g -= t @ LA.inv(sigma_total2I) @ (t.T)
    g = g / 2
    return g
...
for i in range(np.size(res1)):
    if (res1[i] == C_dev[i]):
        count1 = count1 + 1
print("case 3-Naive Bayes with C = \sigma^2*I:- " + str(count1 / np.size(res1) * 100))
###########################################################################################################
              ###########  plots ###########
...
######################################################################################################
#1 - pdf of given data
Plot_PDFs(X1_train,mu1_train,sigma_total2I,X2_train,mu2_train,sigma_total2I,X3_train,mu3_train,sigma_total2I)
plt.show()
Plot_Contours(X1_train,mu1_train,sigma_total2I,X2_train,mu2_train,sigma_total2I,X3_train,mu3_train,sigma_total2I)
plt.show()


#2-scatter+decision+contour
Plot_Contours(X1_train,mu1_train,sigma_total2I,X2_train,mu2_train,sigma_total2I,X3_train,mu3_train,sigma_total2I)
plot_decision(mu1_train,mu2_train,sigma_total2I,10,40)
plot_decision(mu2_train,mu3_train,sigma_total2I,10,40)
plot_decision(mu1_train,mu3_train,sigma_total2I,10,40)
...
#4-decision+pdf
Plot_PDFs(X1_train,mu1_train,sigma_total2I,X2_train,mu2_train,sigma_total2I,X3_train,mu3_train,sigma_total2I)
plot_decision(mu1_train,mu2_train,sigma_total2I,-5,12)
plot_decision(mu2_train,mu3_train,sigma_total2I,7,20)
plot_decision(mu1_train,mu3_train,sigma_total2I,-5,12)
plt.show()

#eigenvectors
Plot_Contours(X1_train,mu1_train,sigma_total2I,X2_train,mu2_train,sigma_total2I,X3_train,mu3_train,sigma_total2I)
Plot_EigenVetors(mu1_train,sigma_total2I)
Plot_EigenVetors(mu2_train,sigma_total2I)
Plot_EigenVetors(mu3_train,sigma_total2I)
plt.show()
