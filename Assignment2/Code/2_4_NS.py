...
def calculate_mu_sigma(c):
    ...
    p = np.diag(sigma)
    sigma1 = np.array([[p[0], 0.], [0., p[1]]])
    return ((x_mean, y_mean), sigma1)
...
########################################################################################################################################
#case 4:- Naive Bayes with C1 = C2 = C3.
...
for i in range(np.size(res1)):
    if (res1[i] == C_dev[i]):
        count1 = count1 + 1
print("case 4 -Naive Bayes with C same for all classes:- " + str(count1 / np.size(res1) * 100))
####################################################################################################
              ###########  plots ###########
...
Plot_Contours(X1_train,mu1_train,sigma_total,X2_train,mu2_train,sigma_total,X3_train,mu3_train,sigma_total)
plot_decision(mu1_train,mu2_train,sigma_total,10,40)
plot_decision(mu2_train,mu3_train,sigma_total,10,40)
plot_decision(mu1_train,mu3_train,sigma_total,10,40)
