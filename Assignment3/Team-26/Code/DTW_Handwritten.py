...
directories_train = ["./ai/train","./bA/train","./chA/train","./dA/train","./tA/train"]
directories_dev = ["./ai/dev","./bA/dev","./chA/dev","./dA/dev","./tA/dev"]
...
def find_output(X_dev,original_class):
    ...
    for dev_point in X_dev:
        ...
        for train_point in X_train_ai:
            dist_arr1.append(dtw(train_point,dev_point))
        for train_point in X_train_bA:
            dist_arr2.append(dtw(train_point,dev_point))
        for train_point in X_train_chA:
            dist_arr3.append(dtw(train_point,dev_point))
        for train_point in X_train_dA:
            dist_arr4.append(dtw(train_point,dev_point))
        for train_point in X_train_tA:
            dist_arr5.append(dtw(train_point,dev_point))

        k = 2
        dist_arr1, dist_arr2, dist_arr3, dist_arr4, dist_arr5 = sorted(dist_arr1), sorted(dist_arr2), sorted(
            dist_arr3), sorted(dist_arr4), sorted(dist_arr5)
        scores_i = [k / sum(dist_arr1[:k]), k / sum(dist_arr2[:k]), k / sum(dist_arr3[:k]), k / sum(dist_arr4[:k]),
                    k / sum(dist_arr5[:k])]
...
(a,b)=find_output(X_dev_ai,0)
(c,d)=find_output(X_dev_bA,1)
(e,f)=find_output(X_dev_chA,2)
(g,h)=find_output(X_dev_dA,3)
(i,j)=find_output(X_dev_tA,4)

print("Accuracy is :-" + str((a+c+e+g+i)/(b+d+f+h+j) * 100)+"%.")
C_dev = [0]*len(X_dev_ai)+[1]*len(X_dev_bA)+[2]*len(X_dev_chA)+[3]*len(X_dev_dA)+[4]*len(X_dev_tA)
n = len(C_dev)
...
def plot_ROC_curve(scores, n):
    ...
    plt.plot(fpr,tpr,label=str(lab))
    lab += 1
